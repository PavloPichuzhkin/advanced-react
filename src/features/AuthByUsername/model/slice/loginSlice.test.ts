...
describe('loginSlice.test', () => {
    ...
    test('test sets isLoading = true when loginByUsername is pending', () => {
        const initialState: DeepPartial<LoginSchema> = { password: '123', username: '123', isLoading: false };
        const action = { type: loginByUsername.pending.type };
        expect(loginReducer(initialState as LoginSchema, action))
            .toEqual({ password: '123', username: '123', isLoading: true });
    });

    test('test sets error when loginByUsername is rejected', () => {
        const initialState: DeepPartial<LoginSchema> = { username: '123', isLoading: true };
        const action = { type: loginByUsername.rejected.type, payload: 'some error' };
        expect(loginReducer(initialState as LoginSchema, action))
            .toEqual({ username: '123', isLoading: false, error: 'some error' });
    });

    test('test username and password will not change when loginByUsername is fulfilled', () => {
        const initialState: DeepPartial<LoginSchema> = { username: '123', password: '123', isLoading: true };
        const action = {
        ...
        expect(loginReducer(initialState as LoginSchema, action))
            .toEqual({
                ...
                password: '123', username: '123', isLoading: false, error: undefined,
            });
    });
});
